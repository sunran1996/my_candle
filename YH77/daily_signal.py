# -*- coding: utf-8 -*-
"""
个股交易策略 YH77: 综合 YH08 + YH09 的 8 支标的与策略
标的: 长江电力 招商银行 国电电力 中国神华 (YH08) + 山东高速 渝农商行 皖通高速 江苏银行 (YH09)
牛熊因子(现价 vs MA120)
"""
...
CORE_STOCKS = {'长江电力': 'sh600900', '招商银行': 'sh600036', '国电电力': 'sh600795', '中国神华': 'sh601088',
               '山东高速': 'sh600350', '渝农商行': 'sh601077', '皖通高速': 'sh600012', '江苏银行': 'sh600919'}
ALL_STOCKS = dict(CORE_STOCKS)

INIT = 1_000_000; COMM = 0.0003; SLIP = 0.0001; MAX_POS = 0.125   # 每只票等权 1/8
# 每只核心股默认仓位上限: 等权 1/8 (连亏时小幅加仓, 见 MAX_POS_BOOST/DOUBLE)
BASE_POS = {}
...
#   YH08 标的: 2026-09 调优, 牛市低止盈(5~8%)快进快出, 熊市高止盈(40%)让移动止损做主 (样本外验证通过)
#   YH09 标的: 样本外未通过, 保持 YH-1.0 基线 (牛熊同参)
BUY_PARAMS = {
    'bull': {
        # ── YH08 标的 ──
...
MAX_POS_BOOST  = 0.15       # 连亏≥2 + 有其他持仓 → 小幅加仓到15%
MAX_POS_DOUBLE = 0.20       # 连亏≥4 + 有其他持仓 → 加仓到20%
...
def push_state(token):
    ...
    try:
        github_put(token, 'YH77/_positions.json', b64, 'YH77 position state')
        print(f'  持仓状态已推送')
    except Exception as e:
        print(f'  状态推送失败: {e}')
...
def add_indicators(df):
    ...
    df['ma20'] = c.rolling(20).mean(); df['ma50'] = c.rolling(50).mean(); df['ma60'] = c.rolling(60).mean()
    df['ma90'] = c.rolling(90).mean(); df['ma120'] = c.rolling(120).mean()
    df['ma150'] = c.rolling(150).mean(); df['ma180'] = c.rolling(180).mean()
...
REGIME_MA = 50   # 牛熊分界 MA 周期 (对齐 YH08: MA50 样本外最优)
...
def check_buy(row, name):
    """每只股票独立阈值, score>=1; 无RSI/BB参数返回False"""
...
def push_alerts(token):
    ...
    b64 = base64.b64encode(raw).decode('ascii')
    try:
        github_put(token, 'YH77/_alerts_sent.json', b64, 'YH77 alert dedup state')
        print(f'  提醒状态已推送')
    except Exception as e:
        print(f'  提醒状态推送失败: {e}')
...
def run_backtest(start_str=None, inject=True):
    ...
    print(f"\n{'='*60}")
    print(f"  YH77: TS={TRAIL_STOP*100:.0f}%+止损冷却{COOLDOWN}天 | 月投{MONTHLY_INJECT/10000:.0f}w")
    print(f"  {'─'*40}")
    ...
    print(f"\n  {'年份':<6} {'组合':>8} {'回撤':>7}  {'长江电力':>8} {'招商银行':>8} {'国电电力':>8} {'中国神华':>8} {'山东高速':>8} {'渝农商行':>8} {'皖通高速':>8} {'江苏银行':>8}")
    for yr, grp in ndf.groupby('year'):
        ...
        for name in ALL_STOCKS:
            ...
            if len(ss) > 0:
                ...
                parts.append('—')
        print(f"  {yr:<6} {yr_ret:>+7.1f}% {yr_mdd:>+6.1f}%  {parts[0]:>8}  {parts[1]:>8}  {parts[2]:>8}  {parts[3]:>8}  {parts[4]:>8}  {parts[5]:>8}  {parts[6]:>8}  {parts[7]:>8}")
    ...
    ax.set_title(f'YH77 均值回归 | +{ret:.1f}% | 年化{ann:.1f}% | 夏普{sr:.2f} | 回撤{mdd:.1f}% | {len(td)}笔 | 胜率{wr:.0f}%',
...
def plot_equity_curve(ndf, ndf0=None):
    ...
    ax1.set_title('YH77 收益曲线  |  含定投 vs 无定投', fontsize=15, fontweight='bold')
...
def github_put(token, path, content_b64, msg):
    ctx = ssl._create_unverified_context()
    h = {'Authorization':'Bearer '+token, 'User-Agent':'YH77'}
...
def upload_chart(token, img_bytes):
    ...
    fn = f'chart_{ts}.png'
    try:
        github_put(token, f'YH77/{fn}', base64.b64encode(img_bytes).decode('ascii'), 'YH77 v1.0 chart')
    except Exception as e:
        print(f'  图表上传失败: {e}')
        return ''
    return f'https://cdn.jsdelivr.net/gh/{REPO}@main/YH77/{fn}'
...
def push_code(token):
    ...
    # push to YH77/
    try:
        existed = github_put(token, 'YH77/daily_signal.py', b64, 'YH77 v1.0 daily update')
        print(f"  代码已推送 YH77 v1.0 ({'更新' if existed else '新建'})")
    except Exception as e:
        print(f'  代码推送失败: {e}')
...
def live_signal():
        ...
        for n in CORE_STOCKS:
            ...
            if val > 5000:
                ...
                if qty >= 100:
                    ...
                    if pos_limit >= MAX_POS_DOUBLE: label += f' 加仓(连{loss_streak[n]}亏,仓位{real_pct:.0f}%)'
    ...
    if not is_trading_day:
        ...
        if alerts:
            title = f'YH77 [{day_type}] ⚠️ {near_short}'
        elif buy_count >= 1:
            title = f'YH77 [{day_type}] 买入: ' + ' '.join(buy_names)
        elif holding_count > 0:
            title = f'YH77 [{day_type}] 持仓中 ({holding_count}只)'
        else:
            title = f'YH77 [{day_type}] 空仓 (非交易日)'
    else:
        has_sell = any(t['dir'] == 'SELL' for t in new_trades)
        if has_sell:
            title = 'YH77 🔴 ' + ' '.join(parts)
        elif new_trades:
            title = 'YH77 ' + ' '.join(parts)
        elif alerts:
            title = 'YH77 ⚠️ ' + near_short
        elif buy_count >= 3: title = 'YH77 多只买入! ' + ' '.join(buy_names)
        elif buy_count >= 1: title = 'YH77 买入: ' + ' '.join(buy_names)
        elif holding_count > 0: title = f'YH77 持仓中 ({holding_count}只)'
        else: title = 'YH77 空仓观望'
