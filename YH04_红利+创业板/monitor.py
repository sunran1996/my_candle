...
def run_backtest(start_str):
    ...
    if len(ndf) > 1:
        ...
        def to_weekly(mpdf):
            w = mpdf.resample('W').agg({'Open':'first','High':'max','Low':'min',
                                         'Close':'last','Volume':'sum'})
            return w.dropna()

        # ===== 1. 红利低波数据 =====
        main_raw = raw[MAIN_NAME][raw[MAIN_NAME]['date'] >= start].set_index('date')
        main_adj, ratio_main = adj_ohlc(main_raw)
        mpf_main = main_adj.rename(columns={'open':'Open','high':'High','low':'Low',
                                             'close':'Close','volume':'Volume'})
        mpf_main = mpf_main[['Open','High','Low','Close','Volume']]

        # ===== 2. 创业板数据 =====
        gname = '创业板'
        gro_raw = raw[gname][raw[gname]['date'] >= start].set_index('date')
        gro_adj, ratio_gro = adj_ohlc(gro_raw)
        mpf_gro = gro_adj.rename(columns={'open':'Open','high':'High','low':'Low',
                                           'close':'Close','volume':'Volume'})
        mpf_gro = mpf_gro[['Open','High','Low','Close','Volume']]

        # ===== 3. 交易标记 =====
        buy_m = pd.Series(np.nan, index=mpf_main.index)
        sell_m = pd.Series(np.nan, index=mpf_main.index)
        buy_g = pd.Series(np.nan, index=mpf_gro.index)
        sell_g = pd.Series(np.nan, index=mpf_gro.index)
        for _, t in td.iterrows():
            d2 = t['date']
            nm = t.get('name', '')
            if pd.isna(nm): continue
            pv = float(t.get('price',0)) if pd.notna(t.get('price',0)) else 0.0
            if pv <= 0: continue
            is_buy = (t['dir'] == 'BUY')
            if nm == MAIN_NAME and d2 in mpf_main.index:
                pv_adj = pv * float(ratio_main.loc[d2])
                (buy_m if is_buy else sell_m).loc[d2] = pv_adj
            elif nm == gname and d2 in mpf_gro.index:
                pv_adj = pv * float(ratio_gro.loc[d2])
                (buy_g if is_buy else sell_g).loc[d2] = pv_adj

        # ===== 4. 净值数据 =====
        nav_idx = ndf.set_index('date')
        nav_common = mpf_main.index.intersection(nav_idx.index)
        nav_y = nav_idx.loc[nav_common, 'nav'] / INIT

        # ===== 5. 周线 + BB (BB单独存储, 不混入OHLCV) =====
        mpf_main_w = to_weekly(mpf_main)
        cs_w = mpf_main_w['Close']
        bb_ma = cs_w.rolling(BB_P).mean()
        bb_std = cs_w.rolling(BB_P).std()
        bb_up = bb_ma + BB_S * bb_std
        bb_lo = bb_ma - BB_S * bb_std
        mpf_gro_w = to_weekly(mpf_gro)

        def map_trades_w(trades_s, weekly_idx):
            """映射交易日到所属周(周日, 与resample('W')对齐)"""
            w = pd.Series(np.nan, index=weekly_idx)
            for d, v in trades_s.dropna().items():
                wk = d + pd.Timedelta(days=(6 - d.dayofweek))  # →周日
                if wk in weekly_idx: w.loc[wk] = v
            return w
        buy_m_w = map_trades_w(buy_m, mpf_main_w.index)
        sell_m_w = map_trades_w(sell_m, mpf_main_w.index)
        buy_g_w = map_trades_w(buy_g, mpf_gro_w.index)
        sell_g_w = map_trades_w(sell_g, mpf_gro_w.index)

        # ===== 6. 样式 =====
        mc = mpf.make_marketcolors(up='#CC0000', down='#008800', edge='inherit',
                                    wick='inherit', volume='inherit')
        style = mpf.make_mpf_style(marketcolors=mc, gridstyle='',
                                    rc={'font.sans-serif':[CN], 'axes.unicode_minus':False})

        dca_label = f' | DCA月投{DCA/1e4:.0f}万' if DCA > 0 else ''
        mtitle = (f'YH04 {dca_label}  |  '
                  f'收益 {ret:+.1f}%  |  年化 {ann:+.1f}%  |  夏普 {sr:.2f}  |  '
                  f'最大回撤 {mdd:+.1f}%  |  {len(td)}笔交易')

        # ===== 7. 图1: 红利低波周K + BB + 净值 =====
        ap_ma  = mpf.make_addplot(bb_ma.values, color='#888888', width=0.8, linestyle='--')
        ap_up  = mpf.make_addplot(bb_up.values, color='#9B59B6', width=0.6, linestyle='--')
        ap_lo  = mpf.make_addplot(bb_lo.values, color='#9B59B6', width=0.6, linestyle='--')
        ap_bm  = mpf.make_addplot(buy_m_w.values, type='scatter', marker='^', markersize=80,
                                   color='#FF0000', alpha=0.9)
        ap_sm  = mpf.make_addplot(sell_m_w.values, type='scatter', marker='v', markersize=80,
                                   color='#008800', alpha=0.9)
        # 净值(日线)对齐到周线日期用于panel=1展示
        nav_daily = nav_idx['nav'] / INIT
        nav_weekly = nav_daily.reindex(mpf_main_w.index, method='ffill').bfill()
        ap_nav = mpf.make_addplot(nav_weekly.values, panel=1, color='#CC2222', width=1.8)
        ap_one = mpf.make_addplot(np.ones(len(mpf_main_w)), panel=1, color='#888888',
                                   width=0.6, linestyle='--')

        fig1, axes1 = mpf.plot(mpf_main_w, type='candle', style=style,
                                addplot=[ap_ma, ap_up, ap_lo, ap_bm, ap_sm, ap_nav, ap_one],
                                volume=False, title=mtitle,
                                ylabel='红利低波(周)', ylabel_lower='净值(倍)',
                                returnfig=True, figsize=(20, 11),
                                panel_ratios=(2.8, 1.3),
                                datetime_format='%Y-%m')
        fig1.savefig(os.path.join(SCRIPT, 'backtest_chart.png'), dpi=150,
                     bbox_inches='tight', facecolor='white')
        plt.close(fig1)

        # ===== 8. 图2: 创业板周K + 买卖点 =====
        ap_bg = mpf.make_addplot(buy_g_w.values, type='scatter', marker='^', markersize=80,
                                  color='#FF0000', alpha=0.9)
        ap_sg = mpf.make_addplot(sell_g_w.values, type='scatter', marker='v', markersize=80,
                                  color='#008800', alpha=0.9)
        gtitle = f'YH04 {gname} (成长轮动·周线)  |  买卖标记: ▲买入 ▼卖出'
        fig2, axes2 = mpf.plot(mpf_gro_w, type='candle', style=style,
                                addplot=[ap_bg, ap_sg], volume=False,
                                title=gtitle, ylabel=gname,
                                returnfig=True, figsize=(20, 7),
                                datetime_format='%Y-%m')
        fig2.savefig(os.path.join(SCRIPT, 'growth_chart.png'), dpi=150,
                     bbox_inches='tight', facecolor='white')
        plt.close(fig2)

        print(f'  收益图: {SCRIPT}/backtest_chart.png')
        print(f'  创业板图: {SCRIPT}/growth_chart.png')
        # 导出净值CSV
        ndf[['date','nav']].to_csv(os.path.join(SCRIPT,'_nav_yh04.csv'), index=False)
        print(f'  净值导出: {SCRIPT}/_nav_yh04.csv')
    ...
    if len(ndf) > 1:
        ...
        for yr, grp in ndf.groupby('year'):
            if len(grp) < 10: continue
            years.append(yr)
            rets.append((grp['nav'].iloc[-1] / grp['nav'].iloc[0] - 1) * 100)
        ...
        ax.set_xticklabels([str(y) for y in years], fontsize=13, fontweight='bold')
        ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%+.0f%%'))
        ax.set_ylabel('年度收益', fontsize=12)
        ax.grid(True, alpha=0.12, axis='y')
        ax.set_title('YH04 年度收益', fontsize=14, fontweight='bold')
        plt.savefig(os.path.join(SCRIPT, 'nav_chart.png'), dpi=150,
                    bbox_inches='tight', facecolor='white')
        plt.close()
        print(f'  年度收益图: {SCRIPT}/nav_chart.png')
    ...
    if len(ndf) > 1 and len(td) > 0:
        ...
        fig, axes = plt.subplots(5, 1, figsize=(16, 16), facecolor='white',
                                 gridspec_kw={'hspace': 0.1, 'top': 0.96, 'bottom': 0.02})
        ...
        for i, (name, ax) in enumerate(zip(names, axes)):
            ...
            for _, t in td.iterrows():
                ...
                if t['dir'] == 'BUY':
                    ax.scatter(d2, pv2, color='#CC2222', s=70, marker='^', zorder=5,
                              edgecolors='white', lw=1.2)
                else:
                    ax.scatter(d2, pv2, color='#228B22', s=70, marker='v', zorder=5,
                              edgecolors='white', lw=1.2)
        ...
        axes[0].set_title(f'YH04 交易全景{dca_label} | {ret:+.1f}% | 夏普{sr:.3f} | {len(td)}笔',
                         fontsize=14, fontweight='bold')
        plt.savefig(os.path.join(SCRIPT, 'trades_chart.png'), dpi=120,
                    bbox_inches='tight', facecolor='white')
        plt.close()
