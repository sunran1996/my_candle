...
BUY_PARAMS = {
    '伊利股份': {'rsi': 35, 'bb': 0.12, 'tp': 0.15, 'tp_hi': 0.20},
...
def run_backtest(start_str=None):
    ...
    print(f"\n{'='*60}")
    print(f"  YH-1.1: TS={TRAIL_STOP*100:.0f}%+止损冷却{COOLDOWN}天 | 月投{MONTHLY_INJECT/10000:.0f}w | 6核心")
    print(f"  {'─'*40}")
    ...
    names_list = list(ALL_STOCKS.keys())
    header = f"\n  {'年份':<6} {'组合':>8} {'回撤':>7}  " + '  '.join(f'{n:>8}' for n in names_list)
    print(header)
    for yr, grp in ndf.groupby('year'):
        ...
        for name in names_list:
            ...
            if len(ss) > 0:
                ...
                parts.append('—')
        p_str = '  '.join(f'{p:>8}' for p in parts)
        print(f"  {yr:<6} {yr_ret:>+7.1f}% {yr_mdd:>+6.1f}%  {p_str}")

    # ── 图表 ──
    RED = '#CC0000'; GREEN = '#008800'; PURPLE = '#9B59B6'; BLUE = '#3498DB'
    ORANGE = '#E67E22'; GRAY = '#888888'
    colors2 = [RED, PURPLE]
    ...
    gs = fig.add_gridspec(1 + n_stocks + 1, 1, height_ratios=[1.2] + [2.2]*n_stocks + [1.0],
    ...
    ax.set_title(f'YH-1.1 均值回归+创业板补位(6核心) | +{ret:.1f}% | 年化{ann:.1f}% | 夏普{sr:.2f} | 回撤{mdd:.1f}% | {len(td)}笔 | 胜率{wr:.0f}%',
    ...
    for idx, (name, color) in enumerate(zip(ALL_STOCKS.keys(), colors2)):
    ...
    ax = fig.add_subplot(gs[n_stocks + 1])
...
def github_put(token, path, content_b64, msg):
    ...
    h = {'Authorization':'Bearer '+token, 'User-Agent':'YH-1.1'}
...
def upload_chart(token, img_bytes):
    ...
    fn = f'chart_{ts}.png'
    try:
        github_put(token, f'YH-1.1/{fn}', base64.b64encode(img_bytes).decode('ascii'), 'YH-1.1 v1.0 chart')
    except Exception as e:
        print(f'  图表上传失败: {e}')
        return ''
    return f'https://cdn.jsdelivr.net/gh/{REPO}@main/YH-1.1/{fn}'
...
def push_code(token):
    ...
    # push to YH-1.1/
    try:
        existed = github_put(token, 'YH-1.1/daily_signal.py', b64, 'YH-1.1 v1.0 daily update')
        print(f"  代码已推送 YH-1.1 v1.0 ({'更新' if existed else '新建'})")
    except Exception as e:
        print(f'  代码推送失败: {e}')
...
def live_signal():
    ...
    if not is_trading_day:
        ...
        if buy_count >= 1:
            title = f'YH1.1 [{day_type}] 买入: ' + ' '.join(buy_names)
        elif holding_count > 0:
            title = f'YH1.1 [{day_type}] 持仓中 ({holding_count}只)'
        else:
            title = f'YH1.1 [{day_type}] 空仓 (非交易日)'
    else:
        if buy_count >= 3: title = 'YH1.1 多只买入! ' + ' '.join(buy_names)
        elif buy_count >= 1: title = 'YH1.1 买入: ' + ' '.join(buy_names)
        elif holding_count > 0: title = f'YH1.1 持仓中 ({holding_count}只)'
        else: title = 'YH1.1 空仓观望'
