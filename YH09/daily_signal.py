# -*- coding: utf-8 -*-
"""
个股交易策略 YH09: 从YH-1.0 fork + 牛熊因子(现价 vs MA120 分牛熊两套参数)
...
# 当前牛/熊暂用同一套值(从YH-1.0继承), 待后续分状态调优
BUY_PARAMS = {
    'bull': {
...
def push_state(token):
    ...
    try:
        github_put(token, 'YH09/_positions.json', b64, 'YH09 position state')
        print(f'  持仓状态已推送')
    except Exception as e:
        print(f'  状态推送失败: {e}')
...
def push_alerts(token):
    ...
    b64 = base64.b64encode(raw).decode('ascii')
    try:
        github_put(token, 'YH09/_alerts_sent.json', b64, 'YH09 alert dedup state')
        print(f'  提醒状态已推送')
    except Exception as e:
        print(f'  提醒状态推送失败: {e}')
...
def run_backtest(start_str=None, inject=True):
    ...
    print(f"\n{'='*60}")
    print(f"  YH09: TS={TRAIL_STOP*100:.0f}%+止损冷却{COOLDOWN}天 | 月投{MONTHLY_INJECT/10000:.0f}w")
    print(f"  {'─'*40}")
    ...
    ax.set_title(f'YH09 均值回归+创业板补位 | +{ret:.1f}% | 年化{ann:.1f}% | 夏普{sr:.2f} | 回撤{mdd:.1f}% | {len(td)}笔 | 胜率{wr:.0f}%',
...
def plot_equity_curve(ndf, ndf0=None):
    ...
    ax1.set_title('YH09 收益曲线  |  含定投 vs 无定投', fontsize=15, fontweight='bold')
...
def github_put(token, path, content_b64, msg):
    ctx = ssl._create_unverified_context()
    h = {'Authorization':'Bearer '+token, 'User-Agent':'YH09'}
...
def upload_chart(token, img_bytes):
    ...
    fn = f'chart_{ts}.png'
    try:
        github_put(token, f'YH09/{fn}', base64.b64encode(img_bytes).decode('ascii'), 'YH09 v1.0 chart')
    except Exception as e:
        print(f'  图表上传失败: {e}')
        return ''
    return f'https://cdn.jsdelivr.net/gh/{REPO}@main/YH09/{fn}'
...
def push_code(token):
    ...
    # push to YH09/
    try:
        existed = github_put(token, 'YH09/daily_signal.py', b64, 'YH09 v1.0 daily update')
        print(f"  代码已推送 YH09 v1.0 ({'更新' if existed else '新建'})")
    except Exception as e:
        print(f'  代码推送失败: {e}')
...
def live_signal():
    ...
    if not is_trading_day:
        ...
        if alerts:
            title = f'YH09 [{day_type}] ⚠️ {near_short}'
        elif buy_count >= 1:
            title = f'YH09 [{day_type}] 买入: ' + ' '.join(buy_names)
        elif holding_count > 0:
            title = f'YH09 [{day_type}] 持仓中 ({holding_count}只)'
        else:
            title = f'YH09 [{day_type}] 空仓 (非交易日)'
    else:
        has_sell = any(t['dir'] == 'SELL' for t in new_trades)
        if has_sell:
            title = 'YH09 🔴 ' + ' '.join(parts)
        elif new_trades:
            title = 'YH09 ' + ' '.join(parts)
        elif alerts:
            title = 'YH09 ⚠️ ' + near_short
        elif buy_count >= 3: title = 'YH09 多只买入! ' + ' '.join(buy_names)
        elif buy_count >= 1: title = 'YH09 买入: ' + ' '.join(buy_names)
        elif holding_count > 0: title = f'YH09 持仓中 ({holding_count}只)'
        else: title = 'YH09 空仓观望'
