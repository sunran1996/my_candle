# -*- coding: utf-8 -*-
"""
个股交易策略 YH-1.2: YH-1.0 盘中监控循环版 (死循环抓买卖点 + Bark推送, 只提醒不成交)
...
# 卖出 (默认值)
TRAIL_STOP     = 0.08       # 移动止损8% (最高价口径下最优)
...
def push_state(token):
    ...
    try:
        github_put(token, 'YH-1.2/_positions.json', b64, 'YH-1.2 position state')
        print(f'  持仓状态已推送')
    except Exception as e:
        print(f'  状态推送失败: {e}')
...
WATCH_FILE = os.path.join(SCRIPT, '_watch.json')

def _load_watch():
    """加载手动维护的持仓观察表 _watch.json (无文件返回空 dict)"""
...
def push_alerts(token):
    ...
    b64 = base64.b64encode(raw).decode('ascii')
    try:
        github_put(token, 'YH-1.2/_alerts_sent.json', b64, 'YH-1.2 alert dedup state')
        print(f'  提醒状态已推送')
    except Exception as e:
        print(f'  提醒状态推送失败: {e}')
...
def run_backtest(start_str=None, inject=True):
    ...
    print(f"\n{'='*60}")
    print(f"  YH-1.2: TS={TRAIL_STOP*100:.0f}%+止损冷却{COOLDOWN}天 | 月投{MONTHLY_INJECT/10000:.0f}w")
    print(f"  {'─'*40}")
    ...
    ax.set_title(f'YH-1.2 均值回归+创业板补位 | +{ret:.1f}% | 年化{ann:.1f}% | 夏普{sr:.2f} | 回撤{mdd:.1f}% | {len(td)}笔 | 胜率{wr:.0f}%',
...
def plot_equity_curve(ndf, ndf0=None):
    ...
    ax1.set_title('YH-1.2 收益曲线  |  含定投 vs 无定投', fontsize=15, fontweight='bold')
...
def github_put(token, path, content_b64, msg):
    ctx = ssl._create_unverified_context()
    h = {'Authorization':'Bearer '+token, 'User-Agent':'YH-1.2'}
...
def upload_chart(token, img_bytes):
    ...
    fn = f'chart_{ts}.png'
    try:
        github_put(token, f'YH-1.2/{fn}', base64.b64encode(img_bytes).decode('ascii'), 'YH-1.2 v1.0 chart')
    except Exception as e:
        print(f'  图表上传失败: {e}')
        return ''
    return f'https://cdn.jsdelivr.net/gh/{REPO}@main/YH-1.2/{fn}'
...
def push_code(token):
    ...
    # push to YH-1.2/
    try:
        existed = github_put(token, 'YH-1.2/daily_signal.py', b64, 'YH-1.2 v1.0 daily update')
        print(f"  代码已推送 YH-1.2 v1.0 ({'更新' if existed else '新建'})")
    except Exception as e:
        print(f'  代码推送失败: {e}')
...
# =====================================================
# 盘中监控循环: 死循环抓买卖点 + Bark推送 (只提醒, 不成交)
# =====================================================
LOOP_INTERVAL_SEC = 300   # 盘中轮询间隔(秒), 默认5分钟
_hist_cache = None        # 全量历史缓存 {name: DataFrame}
_hist_cache_date = None   # 缓存对应的日期(跨日自动刷新)

def _get_history(today):
    """开盘当日首次拉全量历史并缓存, 盘中复用(只更新今天实时K线)"""
    global _hist_cache, _hist_cache_date
    if _hist_cache is None or _hist_cache_date != today:
        print('  刷新历史缓存...')
        _hist_cache = fetch()
        _hist_cache_date = today
    return {n: df.copy() for n, df in _hist_cache.items()}

def _in_session(now):
    """是否盘中交易时段: 工作日 9:30-11:30 / 13:00-15:00"""
    if now.dayofweek >= 5:
        return False
...
def check_once():
    """抓一次买卖点, 有信号则Bark推送(同标的冷却期内去重)"""
    now = pd.Timestamp.now()
    today = now.normalize()
    raw = _get_history(today)
    rt = fetch_realtime(ALL_STOCKS)

    # 注入今天实时价 → 今天这根K线 (缓存不含今天, 盘中每次只更新这一根)
    for name in ALL_STOCKS:
        info = rt.get(name)
        if not info or info.get('price', 0) <= 0:
            continue
        if any(d.normalize() == today for d in raw[name]['date']):
            continue
        price = info['price']
        raw[name] = pd.concat([raw[name], pd.DataFrame([{
            'date': today,
            'open': info.get('open', price) or price,
            'high': info.get('high', price) or price,
            'low':  info.get('low',  price) or price,
            'close': price, 'volume': 0,
        }])], ignore_index=True)
    dfs = {n: add_indicators(d) for n, d in raw.items()}

    # 持仓从 _watch.json 手动观察表读 (只提醒不成交, 不读历史回测状态)
    watch = _load_watch()
    shares = {n: (1.0 if n in watch else 0.0) for n in ALL_STOCKS}
    entry = {n: float(watch[n].get('entry', 0)) if n in watch else 0.0 for n in ALL_STOCKS}
    high = {n: float(watch[n].get('high', 0)) if n in watch else 0.0 for n in ALL_STOCKS}
    accel = {n: bool(watch[n].get('accel', False)) if n in watch else False for n in ALL_STOCKS}
    cooldown = {n: 0 for n in ALL_STOCKS}
    loss_streak = {n: 0 for n in ALL_STOCKS}

    signals = []   # (短标签, 详情)
    for name in ALL_STOCKS:
        r = dfs[name][dfs[name]['date'].dt.normalize() == today]
        if len(r) == 0:
            continue
        row = r.iloc[0]
        cp = row['close']
        holding = shares[name] > 0

        # 接近买卖点
        pa = proximity_alert(row, name, holding, entry[name], high[name], accel[name], cooldown[name])
        if pa:
            signals.append(pa)
        ...
        if holding:
            if entry[name] <= 0:
                continue
            # 用今日最高价抬高移动止损基准(创新高则更新)
            if row['high'] > high[name]:
                high[name] = row['high']
            pnl = cp / entry[name] - 1
            dd = cp / high[name] - 1 if high[name] > 0 else 0
            tp = BUY_PARAMS[name].get('tp', 0.15)
            tp_hi = BUY_PARAMS[name].get('tp_hi', 0.20)
            if pnl <= -HARD_STOP:
                signals.append((f'卖{name}', f'🔴 硬止损 {name} 现{cp:.2f} 成本{entry[name]:.2f} ({pnl*100:+.1f}%)'))
            elif accel[name]:
                if pnl >= tp_hi:
                    signals.append((f'卖{name}', f'🔴 加速止盈 {name} 现{cp:.2f} ({pnl*100:+.1f}%)'))
                elif dd <= -TRAIL_STOP:
                    signals.append((f'卖{name}', f'🔴 加速锁盈 {name} 现{cp:.2f} ({pnl*100:+.1f}%)'))
            elif dd <= -TRAIL_STOP:
                signals.append((f'卖{name}', f'🔴 移动止损 {name} 现{cp:.2f} ({pnl*100:+.1f}%)'))
            elif pnl >= tp:
                signals.append((f'卖{name}', f'🟡 触及止盈 {name} 现{cp:.2f} ({pnl*100:+.1f}%)'))
        else:
            if cooldown[name] > 0:
                continue
            if name == '创业板':
                ...
                if (not pd.isna(dif)) and (not pd.isna(dea)) and dif > dea:
                    signals.append((f'买{name}', f'🟢 MACD金叉 {name} 现{cp:.2f}'))
            else:
                ok, sc = check_buy(row, name)
                if ok:
                    signals.append((f'买{name}', f'🟢 买点 {name} RSI{row["rsi"]:.0f} 评{sc} 现{cp:.2f}'))

    # 创新高后把移动止损基准写回观察表
    if watch:
        dirty = False
        for n in watch:
            if shares[n] > 0 and high[n] > 0 and high[n] != float(watch[n].get('high', 0)):
                watch[n]['high'] = high[n]
                dirty = True
        if dirty:
            _save_watch(watch)
    ...
    sent_map = _load_sent_alerts()
    fresh = []
    for short, detail in signals:
        last = sent_map.get(short)
        if last:
            try:
                if (now - pd.Timestamp(last)).total_seconds() < ALERT_COOLDOWN_MIN * 60:
                    continue
            except Exception:
                pass
        fresh.append((short, detail))
        sent_map[short] = now.strftime('%Y-%m-%d %H:%M:%S')
    if fresh:
        _save_sent_alerts(sent_map)
    ...
    held = []
    for n in ALL_STOCKS:
        if shares[n] > 0 and entry[n] > 0:
            r2 = dfs[n][dfs[n]['date'].dt.normalize() == today]
            if len(r2) > 0:
                pnl = (r2.iloc[0]['close'] / entry[n] - 1) * 100
                held.append(f'{n}({pnl:+.1f}%)')
    print(f"{now.strftime('%H:%M:%S')} 持仓: " + (', '.join(held) if held else '空仓'))
    if fresh:
        print('  触发信号:')
        for short, detail in fresh:
            print(f'    {detail}')

    # 推送
    if fresh:
        title = 'YH-1.2 ' + ' '.join(s for s, _ in fresh)
...
def monitor_loop():
    print(f"YH-1.2 监控循环启动: 盘中每{LOOP_INTERVAL_SEC//60}分钟抓一次买卖点, 非交易时段休眠到开盘")
    print(f"  标的: {list(ALL_STOCKS)}  只监控提醒, 不自动成交  (Ctrl+C 退出)")
    while True:
        try:
            now = pd.Timestamp.now()
            if not _in_session(now):
                nxt = _next_open(now)
                sleep_sec = max(60.0, min((nxt - now).total_seconds(), 3600.0))
                print(f"  [{now.strftime('%m-%d %H:%M')}] 非交易时段, 休眠 {sleep_sec/60:.0f}分钟 到 {nxt.strftime('%m-%d %H:%M')}")
            ...
            print(f"  [{pd.Timestamp.now().strftime('%H:%M:%S')}] 本轮异常: {e}")
            time.sleep(LOOP_INTERVAL_SEC)
...
def main():
    ...
    p.add_argument('--interval', dest='interval', type=int, default=300, help='监控轮询间隔秒(默认300)')
    a = p.parse_args()
    global LOOP_INTERVAL_SEC
    if a.interval and a.interval > 0:
        LOOP_INTERVAL_SEC = a.interval
    if a.loop:
        monitor_loop()
    elif a.live:
        live_signal()
    else:
        run_backtest(a.fr or '2016-01-01')
