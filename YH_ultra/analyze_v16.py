...
TAKE_PROFIT=0.20;TAKE_PROFIT_HI=0.25;TAKE_PROFIT_LO=0.15;TRAIL_STOP=0.08;HARD_STOP=0.10
BUY_PARAMS={
...
def add_indicators(df):
    ...
    df['bb_lo_d2']=df['bb_lo'].diff().diff()
...
def check_buy(row,name):
    ...
    if rsi<=30:sc+=1
    d2_lo=row.get('bb_lo_d2')
    min_score=2 if (not pd.isna(d2_lo) and d2_lo<0) else 1
    return sc>=min_score,sc
    ...
    for n in STOCKS:
        ...
        if pnl<=-HARD_STOP:do=True;why='hard'
            ...
            if pnl>=TAKE_PROFIT_HI:do=True;why='accel_tp25'
                ...
                if cp<=stop_px:do=True;sell_px=max(cp,floor_px);why='accel_floor'
        elif pnl>=TAKE_PROFIT_LO:
            d2=r.iloc[0].get('bb_up_d2')
            if not pd.isna(d2) and d2<0:do=True;why='tp_decay'
        ...
        if do:
            ...
            trades.append({'date':date,'name':n,'pnl':(sell_px/entry[n]-1)*100,'why':why,'peak':(high[n]/entry[n]-1)*100})
...
print()
for why in ['tp_decay','tp20','accel_tp25','accel_floor','trail','hard']:
    ts=[t for t in trades if t['why']==why]
    if ts:
        pnls=[t['pnl']for t in ts]
        peaks=[t['peak']for t in ts]
        wr=(sum(1 for p in pnls if p>0))/len(pnls)*100
        print(f'  {why:<13}: {len(ts):>2}笔 均盈{np.mean(pnls):>+5.1f}% 均峰{np.mean(peaks):>+5.1f}% 胜率{wr:>.0f}%')

# BB减速止盈明细
print(f'\nBB减速止盈(tp_decay)明细 ({len([t for t in trades if t["why"]=="tp_decay"])}笔):')
for t in [t for t in trades if t['why']=='tp_decay']:
    print(f"  {t['date'].strftime('%Y-%m-%d')} {t['name']} pnl={t['pnl']:+.1f}% peak={t['peak']:+.1f}%")
